...
public class Main : MonoBehaviour {
	...
	void Update () {
		...
		float ballR = ballObj.GetComponent<SpriteRenderer>().bounds.size.x / 2;
		...
		if(ballObj.transform.position.x + ballR > right){
			ballObj.transform.position = new Vector2(right - ballR, ballObj.transform.position.y);
			ballObj.GetComponent<Ball>().vx *= bounce;
		}else if(ballObj.transform.position.x - ballR < left){
			ballObj.transform.position = new Vector2(left + ballR, ballObj.transform.position.y);
			ballObj.GetComponent<Ball>().vx *= bounce;
		}

		if(ballObj.transform.position.y - ballR < bottom){
			ballObj.transform.position = new Vector2(ballObj.transform.position.x, bottom + ballR);
			ballObj.GetComponent<Ball>().vy *= bounce;
		}else if(ballObj.transform.position.y + ballR > top){
			ballObj.transform.position = new Vector2(ballObj.transform.position.x, top - ballR);
			ballObj.GetComponent<Ball>().vy *= bounce;
		}
			
		float boundLeft = lineObj.transform.position.x;
		float boundRight = lineObj.transform.position.x + bar.GetComponent<SpriteRenderer>().bounds.size.x;
		if (ballObj.transform.position.x > boundLeft && ballObj.transform.position.x < boundRight) {
